...
Any flags or arguments following two standalone double-dashes (``-- --``)
at the end of ``pip-compile``'s argument list are passed to ``pip``, e.g.

.. code-block:: bash

    $ pip-compile requirements.in -- -- --retries 10 --timeout 30
...
Any flags or arguments following two standalone double-dashes (``-- --``)
at the end of ``pip-sync``'s argument list are passed to ``pip install``, e.g.

.. code-block:: bash

    $ pip-sync requirements.txt -- -- --no-cache-dir --no-deps
